...
export class MercadoPagoGateway implements PaymentGateway {
	...
	async generatePaymentQRCode(input: {
		...
		try {
			const response = await this.mercadoPagoInstance.post(
				`/instore/orders/qr/seller/collectors/${this.mercadoPagoUserId}/pos/${this.mercadoPagoExternalPosId}/qrs`,
				{
					...
					notification_url: 'https://mango-china-14.webhook.cool',
	...
	async getPaymentDetails(paymentId: string): Promise<any> {
		try {
			const response = await this.mercadoPagoInstance.get(`/v1/payments/${paymentId}`)
