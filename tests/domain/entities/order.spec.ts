...
describe(Order.name, () => {
  ...
  it("should throw if try to set empty order items", () => {
    ...
    expect(() => order.setOrderItems([])).toThrow(
  ...
  it("should throw if try to restore an order with empty order items", () => {
    expect(() =>
      Order.restore("any_id", validCustomer, [], validStatus)
    ).toThrow(new DomainError("Order must have at least one item"));
  });
  ...
  it("should throw if try to transition from RECEIVED to COMPLETED", () => {
    ...
    expect(() => order.complete()).toThrow(
      new DomainError(
        `Can't transition from ${OrderStatus.RECEIVED} to ${OrderStatus.COMPLETED}`
      )
    );
  });
  ...
  it("should throw if try to transition from RECEIVED to READY", () => {
    ...
    expect(() => order.ready()).toThrow(
      new DomainError(
        `Can't transition from ${OrderStatus.RECEIVED} to ${OrderStatus.READY}`
      )
    );
  });
  ...
  it("should throw if try to transition from IN_PREPARATION to COMPLETED", () => {
    ...
    expect(() => order.complete()).toThrow(
      new DomainError(
        `Can't transition from ${OrderStatus.IN_PREPARATION} to ${OrderStatus.COMPLETED}`
      )
    );
  });
  ...
  it("should throw if try to transition from READY to IN_PREPARATION", () => {
    ...
    expect(() => order.prepare()).toThrow(
      new DomainError(
        `Can't transition from ${OrderStatus.READY} to ${OrderStatus.IN_PREPARATION}`
      )
    );
  });
  ...
  it("should throw if try to transition from COMPLETED to IN_PREPARATION", () => {
    ...
    expect(() => order.prepare()).toThrow(
      new DomainError(
        `Can't transition from ${OrderStatus.COMPLETED} to ${OrderStatus.IN_PREPARATION}`
      )
    );
  });
  ...
  it("should throw if try to transition from COMPLETED to READY", () => {
    ...
    expect(() => order.ready()).toThrow(
      new DomainError(
        `Can't transition from ${OrderStatus.COMPLETED} to ${OrderStatus.READY}`
      )
    );
  });
